...
{
visual_odometry::visual_odometry(float threshold_input)
:threshold(threshold_input)
{
    orb_detector = cv::ORB::create();
    orb_detector->setScaleFactor(1.2f);
    orb_detector->setMaxFeatures(2000);
    clahe_filter = cv::createCLAHE();
    orb_matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::BRUTEFORCE);
...
//        clahe_filter->apply(rgb_image, previous_rgb_frame);
...
//        clahe_filter->apply(rgb_image, current_rgb_frame);
...
//            std::cout << matched_current.at(0) << std::endl;
//            std::cout << matched_points_current[0].pt << std::endl << std::endl;
            ...
            {
//                std::cout << matched_current.size() << std::endl;
//                std::cout << previous_pointcloud.size() << std::endl << std::endl;

                cv::solvePnPRansac(previous_pointcloud, matched_current, camera_matrix, cv::noArray(), 
                               incremental_rot, incremental_trans, false, 500, 1.0, 0.99, cv::noArray(), CV_ITERATIVE);
...
//                std::cout << incremental_trans << std::endl << std::endl;
                ...
                incremental_transform = increment;
    ...
    if(!matched_points.empty())
    {
        for(size_t i = 0; i < matched_points.size(); i++)
        {
          if(matched_points[i][0].distance < threshold*matched_points[i][1].distance)
          {
                ...
                matched_points_current.push_back(current_frame_keypoints[matched_points[i][0].queryIdx]);
                matched_points_previous.push_back(previous_frame_keypoints[matched_points[i][0].trainIdx]);

//                std::cout << current_frame_keypoints[matched_points[i][0].queryIdx].pt << std::endl << std::endl;
          }
        }
    }
...
//    std::cout << matched_points_current.size() << std::endl;
    ...
    std::cout << matched_points_current.size() << std::endl << std::endl;
...
void visual_odometry::TransitionToNextTimeStep()
{
    ...
    current_frame_descriptor.release();
...
void visual_odometry::InitializeFirstFrame()
{
    ...
    {
        std::cout << "Visual Odometry was Initialized successfully using the first frame..." << std::endl;
        std::cout << std::endl;
        first_frame = false;
